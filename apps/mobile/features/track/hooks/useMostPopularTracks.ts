...
const fetchUserMostListenedTracks = async (cursor: string | null, data: AdditionalRankedListParams) => await buildPaginatedQuery<Track>(api, {
    entityKey: 'tracks',
    class: Track,
    url: `/track/most-popular${convertObjectToQueryParams(data)}`,
    cursor: cursor
})

const useMostPopularTracks = (data: AdditionalRankedListParams) => useInfiniteQuery({
    queryKey: ['tracks', 'most-popular', data],
...
export default useMostPopularTracks